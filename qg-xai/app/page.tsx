...
export default function HomePage() {
  ...
  return (
    <main id="main" className="min-h-screen bg-hero text-hero-foreground">
      <Navigation
            ...
            >
              <div
                className={`h-3 rounded-full transition-all duration-300 ${
                  index === activeIndex
                    ? 'bg-hero-foreground'
                    : 'bg-hero-foreground/30 group-hover:bg-hero-foreground/50'
                }`}
              />
              {index === activeIndex && isAutoPlaying && !isPaused && (
                <div
                  className="absolute inset-0 bg-hero-foreground/40 rounded-full animate-pulse"
                  style={{
      ...
      <button
        ...
        className="fixed left-8 top-1/2 -translate-y-1/2 z-50 w-14 h-14 rounded-full bg-hero-foreground/10 backdrop-blur-md border border-hero-foreground/20 flex items-center justify-center text-hero-foreground hover:bg-hero-foreground/20 transition-all duration-300 group shadow-xl"
      ...
      <button
        onClick={handleNext}
        className="fixed right-8 top-1/2 -translate-y-1/2 z-50 w-14 h-14 rounded-full bg-hero-foreground/10 backdrop-blur-md border border-hero-foreground/20 flex items-center justify-center text-hero-foreground hover:bg-hero-foreground/20 transition-all duration-300 group shadow-xl"
