  ...
  return (
    <div className="min-h-screen bg-background text-foreground mt-20">
      ...
      <main className="section-container py-16">
        <h1 className="text-4xl font-extrabold text-center mb-2 text-foreground">Careers at SwiftAI</h1>
        <p className="text-center text-lg text-muted-foreground mb-10 max-w-2xl mx-auto">Join our team of innovators, creators, and leaders. Explore our open positions and help shape the future of technology and business.</p>
        <div className="mb-12">
          ...
          <div className="max-w-3xl mx-auto space-y-8">
            {jobs.map(job => (
              <div
                key={job.id}
                className="bg-card rounded-2xl shadow-md p-6 flex flex-col md:flex-row md:items-center md:justify-between gap-4 border border-border hover:shadow-xl hover:-translate-y-1 transition-all duration-200 group"
              >
                <div className="flex items-center gap-4">
                  <span className="text-3xl md:text-4xl select-none">{job.emoji}</span>
                  <div>
                    <div className="text-xl font-semibold text-foreground group-hover:text-primary transition-colors">{job.title}</div>
                    <div className="text-muted-foreground mt-1 mb-2 text-sm">{job.description}</div>
                  </div>
                </div>
                <Button onClick={() => handleApply(job.id, job.title)} className="w-full md:w-auto rounded-full px-8 py-3 text-lg font-bold shadow transition">Apply</Button>
              </div>
            ))}
          </div>
        </div>
        {applying && (
          <div className="max-w-xl mx-auto mt-16 animate-fade-in bg-card rounded-3xl shadow-2xl p-10 border border-border">
            ...
            >
              ...
              <Input
                ...
                className="border border-input rounded-lg transition"
              />
              <Input
                type="email"
                name="email"
              ...
              <div className="flex gap-4">
                <Button type="submit" size="lg" className="w-full font-bold text-lg py-3 rounded-full shadow-lg transition">Submit Application</Button>
                <Button type="button" variant="outline" className="w-full rounded-full" onClick={() => setApplying(null)}>Cancel</Button>
              </div>
              <p className="text-xs text-muted-foreground mt-2 text-center">We respect your privacy. Your information will never be shared.</p>
            </form>
          </div>
        )}
      </main>
